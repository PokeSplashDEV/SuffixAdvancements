...
public class RemovePrefixCommand {
	public LiteralCommandNode<ServerCommandSource> build() {
		return CommandManager.literal("removePrefix")
				.requires(ctx -> {
	...
	public int run(CommandContext<ServerCommandSource> context) {
		...
		try {
			...
			LP.removePrefix(account.getOwner());

			context.getSource().sendMessage(Text.literal("Removed prefix for " + player));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 1;
	...
	public int usage(CommandContext<ServerCommandSource> context) {
		context.getSource().sendMessage(Text.literal("/adv removePrefix <player>"));
		return 1;
	}
}
